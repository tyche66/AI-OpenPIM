"""PILOT_SUPPLIER taxonomy 导入脚本.

用法:
    python app/scripts/import_sunon_taxonomy.py <json_path>

幂等保证:
    - Supplier「PILOT_SUPPLIER」不存在则创建，存在则复用
...
def main() -> None:
    parser = argparse.ArgumentParser(description="导入PILOT_SUPPLIER taxonomy（品类 + 系列）到 PIM")
