...
const filteredItems = computed(() =>
  items.value.filter((item) => {
    ...
    const matchType = currentType.value === 'all' || item.type === currentType.value
...
async function fetchItems() {
  loading.value = true
  try {
    items.value = await mediaApi.list({ search: searchQuery.value, type: currentType.value })
  } finally {
    loading.value = false
  }
}

watch([searchQuery, currentType], fetchItems)
