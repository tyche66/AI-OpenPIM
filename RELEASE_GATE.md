...
> 适用版本：**所有正式版本 `vX.Y.Z`**，与具体版本号无关。
> 适用分支：`main`（`v1.8.0`–`v1.9.0` 实际都是直接基于 `main` 发布的）。
> 当前版本号不在本文件里声明，见 `CHANGELOG.md`。

本文件定义每一次正式发布的门禁，不绑定某个版本号（原来标题写 V1.8.0、正文写「定义 V1.2 的发布门禁」，两处对不上，因此改为版本无关）。任一门禁失败即为 NO-GO，不接受“测试通过但运维未验收”的条件式发布。门禁验证脚本见 `scripts/release_gate.sh`。
